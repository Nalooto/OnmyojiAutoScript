...
class ScriptTask(WQExplore, SecretScriptTask, WantedQuestsAssets):
    ...
    def next_run(self):
        ...
        if time(hour=6) <= now_time < time(hour=18):
            ...
            next_run_datetime = datetime.combine(now_datetime.date() + timedelta(days=0), time(hour=18))
            ...
            next_run_datetime = datetime.combine(now_datetime.date(), time(hour=6))
            next_run_datetime = next_run_datetime + time_delta
